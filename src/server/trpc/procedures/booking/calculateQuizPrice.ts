...
export const calculateQuizPrice = baseProcedure
  .input(
    z.object({
  ...
  .mutation(async ({ input }) => {
    ...
    const discount = await db.discountConfig.findUnique({ where: { id: 1 } });
    ...
    const originalTotalCents = Math.round(totalPrice * 100);
    let discountCents = 0;
    if (discount?.active) {
      if (discount.type === "PERCENT") {
        discountCents = Math.round(originalTotalCents * (discount.value / 100));
      } else {
        discountCents = Math.round(discount.value * 100);
      }
      discountCents = Math.min(discountCents, originalTotalCents);
    }
    const finalTotalCents = Math.max(originalTotalCents - discountCents, 0);
    const finalPrice = Math.round(finalTotalCents) / 100;
