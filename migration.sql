...
-- EmailTemplate type for system notifications
ALTER TABLE "EmailTemplate" ADD COLUMN IF NOT EXISTS "type" TEXT;
...
-- Recurrence + cancellation fee fields
ALTER TABLE "Booking"
  ADD COLUMN IF NOT EXISTS "recurrenceId" TEXT,
  ADD COLUMN IF NOT EXISTS "occurrenceNumber" INTEGER,
  ADD COLUMN IF NOT EXISTS "cancellationFeeApplied" BOOLEAN DEFAULT false;
...
-- Stripe customer id on user
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "stripeCustomerId" TEXT;
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "stripeDefaultPaymentMethodId" TEXT;
-- Two-cleaner support: BookingCleaner junction
CREATE TABLE IF NOT EXISTS "BookingCleaner" (
  "bookingId" INTEGER NOT NULL,
  "cleanerId" INTEGER NOT NULL,
  CONSTRAINT "BookingCleaner_pkey" PRIMARY KEY ("bookingId","cleanerId")
);

ALTER TABLE "BookingCleaner"
  ADD CONSTRAINT "BookingCleaner_bookingId_fkey" FOREIGN KEY ("bookingId") REFERENCES "Booking"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "BookingCleaner"
  ADD CONSTRAINT "BookingCleaner_cleanerId_fkey" FOREIGN KEY ("cleanerId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- Patch: extend PaymentMethod enum for additional tender types (Zelle/Venmo/Other)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'PaymentMethod' AND e.enumlabel = 'ZELLE') THEN
    ALTER TYPE "PaymentMethod" ADD VALUE 'ZELLE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'PaymentMethod' AND e.enumlabel = 'VENMO') THEN
    ALTER TYPE "PaymentMethod" ADD VALUE 'VENMO';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'PaymentMethod' AND e.enumlabel = 'OTHER') THEN
